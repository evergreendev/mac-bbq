    ...
    <nav className="flex flex-col h-full">
              ...
              <div key={i} className="border-b border-brand-primary-700 pb-4">
                ...
                {subItems && subItems.length > 0 && (
                  <div className="flex flex-col gap-2 mt-3 ml-4">
                    {subItems.map((item) => {
                      return <CMSLink key={item.id} appearance="link" className="text-lg text-gray-300 hover:text-white" {...item.link} />
                    })}
                  </div>
                )}
              </div>
      ...
      <div className="mt-auto p-8 bg-brand-primary-800">
        <div className="flex justify-center space-x-6 mt-6">
          <a href="#" className="text-white hover:text-brand-accent-400">
